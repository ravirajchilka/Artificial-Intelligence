...
constexpr std::int16_t NUM_MAX_STEPS = 100;
constexpr std::int16_t NUM_ACTIONS = 2;
constexpr std::int16_t NUM_EPISODES = 1000;
constexpr float LEARNING_RATE = 0.12F; // increased learning rate
...
class Pendulum {
    public : 
		...
		std::pair<float,std::int16_t>return_reward_state(std::int16_t action) {
			...
			float reward = 0;
			std::int16_t state = (bob_state / 10); 
			...
			if(bob_state < 30 || bob_state > 80) {
				//reward = -1.0F; // use this value to punish pendulum for invalid state transitions
			}
...
int main() {
	...
	for(std::int16_t episode = 0; episode < NUM_EPISODES; ++episode)  {
		auto int_dis = std::uniform_int_distribution(4,8); // current state 4 making pendulum start position at 3
		std::int16_t current_state = int_dis(gen);
        //std::int16_t current_state = 4; // current state 4 making pendulum start position at 3
		std::int16_t terminal_state = 110; // rotate pendulum from 30 degrees to 100 degrees
